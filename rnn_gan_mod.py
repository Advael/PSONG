...
#                    "the initial scale of the weights") nnn
...
flags.DEFINE_integer("songlength", 1,               # 200, 500
...
flags.DEFINE_integer("batch_size", 1,                # 10, 80
                   "Batch size.")
...
flags.DEFINE_boolean("end_classification", True, "Classify only in ends of D. Otherwise, does classification at every timestep and mean reduce.")
...
def make_rnn_cell(rnn_layer_sizes,
  ...
  stacked_cells = tf.contrib.rnn.MultiRNNCell(cells, state_is_tuple=state_is_tuple)
  if attn_length:
    stacked_cells = tf.contrib.rnn.AttentionCellWrapper(
        stacked_cells, attn_length, state_is_tuple=state_is_tuple, reuse=reuse)
...
def my_reduce_mean(what_to_take_mean_over):
  ...
  # nothing after this will fucking execute!
...
class RNNGAN(object):
    ...
    with tf.variable_scope('G') as scope:
      ...
      if is_training and FLAGS.keep_prob < 1:
        ...
        cell = make_rnn_cell([FLAGS.hidden_size_g]*FLAGS.num_layers_g,dropout_keep_prob=FLAGS.keep_prob)
         ...
         # NO fucking dropout if not training
      ...
      random_rnninputs = self.modified_inputs
  ...
  def SetgeneratorInputs(self, batch_size, songlength, num_song_features, pixelData = None):
    # manip pixel data here. 
    return tf.random_uniform(shape=[batch_size, songlength, int(FLAGS.random_input_scale*num_song_features)], minval=0.0, maxval=1.0, dtype=data_type())
  ...
  while batch_meta is not None and batch_song is not None:
    ...
    if datasetlabel == 'train' and not pretraining: # and not FLAGS.feature_matching:
      if d_loss == 0.0 and g_loss == 0.0:
        ...
        #print('D train loss is zero. Freezing optimization. G loss: {}'.format(g_loss))
        op_g = tf.no_op()
      elif g_loss == 0.0: 
        #print('G train loss is zero. Freezing optimization. D loss: {}'.format(d_loss))
        op_d = tf.no_op()
      elif g_loss < 2.0 or d_loss < 2.0:
        if g_loss*.7 > d_loss:
          #print('G train loss is {}, D train loss is {}. Freezing optimization of D'.format(g_loss, d_loss))
          op_g = tf.no_op()
    ...
    paddings = tf.concat([fetches, zero_padding], 2)


    p = tf.Print(paddings, [paddings])
    pprint(p)
    '''
    # fetches = tf.reshape(tf.pad(fetches, mode = 'CONSTANT', ), [1, 1, 6400])
    #print(batch_song)
    ...
    if verbose and iters % 10 == 9:
      ...
      if pretraining:
        print("{}: {} (pretraining) batch loss: G: {}, avg loss: G: {}, speed: {} songs/s, avg in graph: {}, avg in python: {}.".format(datasetlabel, iters, g_loss, float(g_losses)/float(iters), songs_per_sec, avg_time_in_graph, avg_time_in_python))
      else:
...
def sample(session, model,loader, batch = False):
  ...
  # testing = tf.Print(fetches, [fetches], 'testing fetches - ',  summarize = 10)
  # print(testing)
  ...
  # # call pong agent here with the midi. 
  # act(midi_patterns)
...
def main(_):
  ...
  # Stripping out non-important shit. 
  synthetic = None
  ...
  with tf.Graph().as_default(), tf.Session(config=tf.ConfigProto(log_device_placement=FLAGS.log_device_placement)) as session:
    with tf.variable_scope("model", reuse=None) as scope:
      scope.set_regularizer(tf.contrib.layers.l2_regularizer(scale=FLAGS.reg_scale))
      # this is done pre training only. only G is pretrained. 
    ...
    if not FLAGS.sample:
      ...
      for i in range(global_step, FLAGS.max_epoch):
        ...
        print("Epoch: {} Learning rate: {}, pretraining: {}".format(i, session.run(m.lr), (i<FLAGS.pretraining_epochs)))
        if i<FLAGS.pretraining_epochs:
          ...
          if FLAGS.pretraining_d:
            try:
              print("Epoch: {} Pretraining loss: G: {}, D: {}".format(i, train_g_loss, train_d_loss))
            except:
              print(train_g_loss)
              print(train_d_loss)
          else:
        ...
        song_data = sample(session, m, loader, batch=True)
        midi_patterns = []
        ...
        if len(stats):
          ...
          with open(statsfilename, 'a') as f:
            f.write('{} {} {}\n'.format(i, ' '.join(['{}'.format(stats[0][key].replace(' ', '_')) for key in stats_keys_string]), ' '.join(['{}'.format(sum([s[key] for s in stats])/float(len(stats))) for key in stats_keys])))
    ...
    song_data = sample(session, m, loader)
    filename = os.path.join(generated_data_dir, 'out-{}-{}-{}.mid'.format(experiment_label, i, datetime.datetime.today().strftime('%Y-%m-%d-%H-%M-%S')))
...
#       # this is done pre training only. only G is pretrained. 
#       m = RNNGAN(is_training=True, num_song_features=num_song_features, num_meta_features=num_meta_features)
#   return m, session, tf, global_step, loader, songlength_ceiling, train_start_time # returns model by reference AFTER pretraining...


# def generateMidi(m, session, tf, global_step, loader, songlength_ceiling, train_start_time):
...
#   #     # this is done pre training only. only G is pretrained. 
#   #     m = RNNGAN(is_training=True, num_song_features=num_song_features, num_meta_features=num_meta_features)
#   # return m, session # returns model by reference AFTER pretraining...
...
# model, session, tf, global_step, loader, songlength_ceiling, train_start_time = RLAgent()
# pprint("done!")
# generateMidi(model, session, tf, global_step, loader, songlength_ceiling, train_start_time)
# pprint("generated!")
